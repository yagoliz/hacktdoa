...
def geodetic2enu(lat, lon, alt, lat_org, lon_org, alt_org):
    ...
    vec=np.array([x-x_org, y-y_org, z-z_org])

    rot1 =  transform.Rotation.from_euler('x', -(90-lat_org), degrees=True).as_matrix()#angle*-1 : left handed *-1
    rot3 =  transform.Rotation.from_euler('z', -(90+lon_org), degrees=True).as_matrix()#angle*-1 : left handed *-1
...
def enu2geodetic(x, y, z, lat_org, lon_org, alt_org):
    ...
    ecef_org=np.array([[x_org,y_org,z_org]])
